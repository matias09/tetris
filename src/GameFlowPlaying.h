...
public:
  ...
  virtual signed int Run(InputHandlerInterface& inpHandler
                       , GraphicHandlerInterface& grpHandler);
  virtual void Pause();
  virtual void Exit();

  // Set
  void SetDificultyGrade(const unsigned int dificulty) 
  { 
    _mDificultyGrade = dificulty; 
  }

  void SetPuntuation(const unsigned int puntuation) 
  { 
    _mPuntuation = puntuation; 
  }

  void SetScore(const unsigned int score) { _mScore = score; }
  void SetThereIsLateralCollision(const bool thereIsCollision) 
  { 
    _mThereIsLateralCollision = thereIsCollision; 
  }

  void SetThereIsBottomCollision(const bool thereIsCollision) 
  { 
    _mThereIsBottomCollision = thereIsCollision; 
  }
...
private:
  ...
  inline void _ExecuteShapeDown();
  inline void _ExecuteShapeRight();
  inline void _ExecuteShapeLeft();
